...
class Simulation:
    ...
    def _simulate_regular_season(
        ...
        for week in range(self.league_settings.current_week, 17+1):
            if week <= self.league_settings.regular_season_end:
                ...
                for team in matchups_sim:
                    ...
                    team.top_score = 1 if team.team_score == max(scores) else 0
                    week_sim[team.team_id] = team
                all_weeks[week] = week_sim
        ...
        for week, team_results in all_weeks.items():
            for tid, team in team_results.items():
    ...
    def _get_final_standings(
        ...
        if wild_card:
            ...
            order.extend(bottom)
            return {tid: standings[tid] for tid in order if tid in standings}
        else:
            # sort all teams by total wins, points
            standings = [
                t[0] for t
                in sorted(
                    standings.items(),
