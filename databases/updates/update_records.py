...
standings_recs = get_standings_records(last_season=season)
...
records = pd.concat([standings_recs, longest_streaks, matchup_recs, tophalf_recs, per_stat_recs, stat_group_records, points_by_position])
...
Database().batch_insert(
    table='records',
    columns=constants.RECORDS_COLUMNS,
    rows=rows,
    upsert=False,
    update_columns=None
)
